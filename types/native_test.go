...
func TestToObject_Func(t *testing.T) {
	exp := &types.Integer{Value: 1}
	fn := func(...types.Object) (types.Object, error) {
		return exp, nil
	}

	obj, err := types.ToObject(fn)
	if err != nil {
		t.Errorf("got %v, expected nil", err)
	}

	act, _ := obj.(types.Func)()
	if act != exp {
...
func TestToNative(t *testing.T) {
	tests := []struct {
		...
		{
			name:  "invalid map",
			input: types.Map{"x": types.Func(nil)},
			err:   true,
		},
		{
			name:  "array",
		...
		{
			name:  "invalid array",
			input: types.Array{types.Func(nil)},
			err:   true,
		},
		{
			name:  "invalid",
